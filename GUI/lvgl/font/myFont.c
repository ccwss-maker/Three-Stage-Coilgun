...
static uint8_t *__user_font_getdata(FSIZE_t offset, UINT size){
    ...
    //如字模已加载到SDRAM,直接返回偏移地址即可如:return (uint8_t*)(sdram_fontddr+offset);
		...
		if(file.flag!=0x01)
		{
			f_open(&file, "0:/Font/Hua_Wen_Fang_Song_Out_11.bin", FA_OPEN_EXISTING | FA_READ);
		}
		
		if(file.flag==0x01)
		{
			f_lseek(&file, offset);		// 寻址
			f_read(&file, __g_font_buf, size, NULL);
		}
...
//字模高度：25
//XBF字体,外部bin文件
lv_font_t Hua_Wen_Fang_Song_Out = {
