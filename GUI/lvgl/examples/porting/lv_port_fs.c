...
static lv_fs_res_t fs_open (lv_fs_drv_t * drv, void * file_p, const char * path, lv_fs_mode_t mode)
{
    ...
    else if(mode == (LV_FS_MODE_WR | LV_FS_MODE_RD))
    {
		...
		FRESULT fres;
		fres = f_open((FIL*)file_p, path, opt_mode);
		if (fres != FR_OK) {
				sprintf(string,"f_open error (%d)\n", fres);
				lv_textarea_add_text(ta1, string);
...
static lv_fs_res_t fs_write(lv_fs_drv_t * drv, void * file_p, const void * buf, uint32_t btw, uint32_t * bw)
{
    FRESULT fres = f_write((FIL*)file_p, buf, btw, bw);
  	if (fres != FR_OK) {
			sprintf(string,"f_write error (%d)\n", fres);
			lv_textarea_add_text(ta1, string);
    	return LV_FS_RES_NOT_IMP;
  	} else
    	return LV_FS_RES_OK;
...
static lv_fs_res_t fs_seek (lv_fs_drv_t * drv, void * file_p, uint32_t pos)
{
    FRESULT fres = f_lseek((FIL*)file_p, pos);
 	 if (fres != FR_OK) {
			sprintf(string,"f_lseek error (%d)\n", fres);
			lv_textarea_add_text(ta1, string);
    	return LV_FS_RES_NOT_IMP;
  	} else
    	return LV_FS_RES_OK;
