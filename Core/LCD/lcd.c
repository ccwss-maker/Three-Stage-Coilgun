...
_lcd_dev lcddev;
//画笔颜色,背景颜色
uint16_t POINT_COLOR = 0x0000,BACK_COLOR = WHITE;  
uint16_t DeviceCode;	 
...
//*************2.4inch ILI9341初始化**********//	
  ...
  LCD_direction(USE_HORIZONTAL);//设置LCD显示方向
	uint8_t buffer[512];
	SD_ReadBlock(buffer, 0x01);
	LCD_LED=buffer[0];//点亮背光	 
	LCD_Clear(BACK_COLOR);//清全屏白色
}
...
void LCD_LVGL_DrawLine(lv_obj_t *obj, signed short x1, signed short y1, signed short x2, signed short y2,lv_color_t color,uint8_t wide)
{
	...
	lv_style_set_line_width(&style_line, LV_STATE_DEFAULT, wide);
	lv_style_set_line_color(&style_line, LV_STATE_DEFAULT, color);
	
//	lv_obj_t * line1;
	obj = lv_line_create(lv_scr_act(), NULL);
	lv_line_set_points(obj, line_points, 2);     /*Set the points*/
	lv_obj_add_style(obj, LV_LINE_PART_MAIN, &style_line);     /*Set the points*/
}